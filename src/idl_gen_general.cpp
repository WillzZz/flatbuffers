...
void GenStruct(StructDef &struct_def, std::string *code_ptr) {
  ...
  //Generate the SizePrefixedRoot method, but not for structs
  if (lang_.language == IDLOptions::kCSharp && !struct_def.fixed) {
      code += "  public static " + struct_def.name + " GetSizePrefixedRoot(ByteBuffer _bb) {\n";
      code += "    _bb.Position += sizeof(int);\n";
      code += "    return ";
      code += FunctionStart('G') + "etRootAs" + struct_def.name;
      code += "(_bb);\n";
      code += "  }\n";
  }
